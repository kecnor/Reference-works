...
public class MP_Bot : MonoBehaviour
{
    ...
    private MP_Bot_Attribute[] attributes;
    ...
    //Scriptable Objects
    private MP_MoveBot moveBot;
    private MP_Characters characters;
    private MP_DrawCard drawCard;
    private MP_Cheating cheating;
    ...
    public MP_Bot_Attribute[] AttributeList { get { return attributes; } }
    ...
    public int[] Reputations { get { return reputations; }}
    ...
    public void Init(string name, string botClass, string botRace, string botAttidute)
    {
        ...
        attributes = new MP_Bot_Attribute[6];
        attributes[0] = new MP_Bot_Attribute(this, Attributes.Strength, 10);
        attributes[1] = new MP_Bot_Attribute(this, Attributes.Agility, 10);
        attributes[2] = new MP_Bot_Attribute(this, Attributes.Intellect, 10);
        attributes[3] = new MP_Bot_Attribute(this, Attributes.Charisma, 10);
        attributes[4] = new MP_Bot_Attribute(this, Attributes.Stamina, 5);
        attributes[5] = new MP_Bot_Attribute(this, Attributes.Health, 6);
        ...
        moveBot = GameObject.Find("ScriptObjects/Multiplayer/MultiplayerMoveBot").GetComponent<MP_MoveBot>();
        drawCard = GameObject.Find("ScriptObjects/Multiplayer/DrawCard").GetComponent<MP_DrawCard>();
        cheating = GameObject.Find("ScriptObjects/Multiplayer/MultiplayerCheat").GetComponent<MP_Cheating>();
    ...
    {
        int topAttribute = 0;
        foreach (MP_Bot_Attribute attribute in attributes)
        {
            ...
            foreach (InventorySlot slot in slots)
            {
                if (IsEmptySlot(slot.Item) && !used)
                {
                    ...
                    foreach (ItemBuff buff in equipableItem.buffs)
                    {
                        ...
                        WriteLogRpc($"{botName} equiped a {equipableItem.name}. {botName}'s new attack power is {attackPower}");
                ...
                else if(!used)
                {
                    if (IsForClass(equipableItem.buffs))
                    {
                        if (FavourableValue(equipableItem.buffs) > FavourableValue(slot.Item.buffs))
                        {
                            ...
                            WriteLogRpc($"{botName} swapped {slot.Item.name} to {equipableItem.name}. {botName}'s new attack power is {attackPower}");
                            slot.Item = equipableItem;
                            used = true;
                        }
                    }
    ...
    {
        foreach (MP_Bot_Attribute characterAttribute in attributes)
        {
    ...
    public void WouldIntervene(GameObject character)
    {
        ...
        if (character.CompareTag("MP_Player"))
        {
            plusLevel = character.GetComponent<MP_Player>().Level;
        }
        else if (character.CompareTag("MP_Bot"))
        {
            plusLevel = character.GetComponent<MP_Bot>().Level;
        }
    ...
    public void Interfier(int monsterAttackPower)
    {
        ...
        if (bestItem != null)
        {
            ...
            if (character.CompareTag("MP_Player"))
            {
                character.GetComponent<MP_Player>().ApplyBuff(bestItem.buffs, repotationID);
            }
            else if (character.CompareTag("MP_Bot"))
            {
                character.GetComponent<MP_Bot>().ApplyBuff(bestItem.buffs, repotationID);
            }
        }
    }
    ...
    private Item GetBestItem(string forWho, int monsterAttackPower)
    {
        ...
        if (character.CompareTag("MP_Player"))
        {
            attribute = GetBestAttribute(character.GetComponent<MP_Player>().PlayerClass);
            attackpower = character.GetComponent<MP_Player>().AttackPower;
        }
        else if (character.CompareTag("MP_Bot"))
        {
            attribute = GetBestAttribute(character.GetComponent<MP_Bot>().botClass);
            attackpower = character.GetComponent<MP_Bot>().AttackPower;
        }
    ...
    public void CaughtOnCheating(int reputationindex)
    {
        ...
        WriteLogRpc($"{botName} has been caught on cheating");
    }

    //Update the bot's attackPower if nessesarry
    public void AttributeModified(MP_Bot_Attribute attribute)
    {
        CalculateAttackPower();
        if (fighting)
        {
    ...
    public void RepleacePlayer(GameObject character)
    {
        characters.SwapCharacters(character, this.gameObject);
        MP_Player player = character.GetComponent<MP_Player>();
        botName = player.Name;
        level = player.Level;
        botClass = player.PlayerClass;
        botRace = player.PlayerRace;
        active = player.Active;
        drawableCards = player.DrawableCards;
        repotationID = player.ReputationID;
        fighting = player.Fighting;
        SetReputation(characters.CharacterList.Count);

        for (int i = 0; i < attributes.Length; i++)
        {
            this.attributes[i].Value.BaseValue = player.attributes[i].Value.BaseValue;
            this.attributes[i].Value.TempValue = player.attributes[i].Value.TempValue;
        }

        for (int i = 0; i < inventory.GetSlots.Length; i++)
        {
            inventory.GetSlots[i] = player.Inventory.GetSlots[i];
        }

        for (int i = 0; i < equipment.GetSlots.Length; i++)
        {
            equipment.GetSlots[i] = player.Equipment.GetSlots[i];
        }
